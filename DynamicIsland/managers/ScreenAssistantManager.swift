...
class ScreenAssistantManager: NSObject, ObservableObject {
    ...
    private func sendToAI(message: String, files: [ScreenAssistantFile], provider: AIModelProvider) {
        ...
        case .custom:
            sendToCustomAPI(message: message, files: files)
        }
    }
    ...
    private func sendToCustomAPI(message: String, files: [ScreenAssistantFile]) {
        let apiKey = Defaults[.customApiKey]
        let endpoint = Defaults[.customEndpoint]

        guard !endpoint.isEmpty else {
            print("❌ ScreenAssistant: No custom endpoint configured")
            addAssistantMessage("Error: No custom endpoint configured. Please set your endpoint in model settings.")
            isLoading = false
            return
        }
        ...
        let modelId: String
        if let selectedId = selectedModel?.id,
           AIModelProvider.custom.supportedModels.contains(where: { $0.id == selectedId }) {
            modelId = selectedId
        } else {
            modelId = Defaults[.customAIModels].first?.id ?? "custom-model"
        }

        let urlString = endpoint.hasSuffix("/chat/completions") ? endpoint : "\(endpoint)/chat/completions"
        guard let url = URL(string: urlString) else {
            print("❌ ScreenAssistant: Invalid custom API URL")
            addAssistantMessage("Error: Invalid custom API URL")
            isLoading = false
            return
        }

        if apiKey.isEmpty {
            print("ℹ️ ScreenAssistant: Sending to custom endpoint without API key")
            performAPIRequest(url: url, requestBody: buildOpenAIRequestBody(message: message, files: files, model: modelId), provider: .custom)
        } else {
            performOpenAIRequest(url: url, requestBody: buildOpenAIRequestBody(message: message, files: files, model: modelId), apiKey: apiKey, provider: .custom)
        }
    }
